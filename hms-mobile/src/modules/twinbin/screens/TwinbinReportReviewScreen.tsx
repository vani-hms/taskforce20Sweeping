...
type Props = NativeStackScreenProps<RootStackParamList, "TwinbinReportReview">;

export default function TwinbinReportReviewScreen({ route, navigation }: Props) {
  ...
  const act = async (next: "APPROVED" | "REJECTED" | "ACTION_REQUIRED") => {
    ...
    setError("");
    try {
      if (next === "APPROVED") await approveTwinbinReport(report.id);
      else if (next === "REJECTED") await rejectTwinbinReport(report.id);
      else await actionRequiredTwinbinReport(report.id);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 24 }}>
      ...
      <View style={styles.card}>
        <Row label="Area" value={report.bin?.areaName} />
        <Row label="Location" value={report.bin?.locationName} />
