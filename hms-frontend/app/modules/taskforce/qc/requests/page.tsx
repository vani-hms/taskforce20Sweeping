'use client';

import { useEffect, useMemo, useState } from "react";
import { Protected, ModuleGuard } from "@components/Guards";
import { ApiError, EmployeesApi, TaskforceApi } from "@lib/apiClient";

type Request = {
  id: string;
  feederPointName: string;
  areaName: string;
  zoneName?: string;
  wardName?: string;
  populationDensity?: string;
  accessibilityLevel?: string;
  householdsCount?: number;
  vehicleType?: string;
  landmark?: string;
  photoUrl?: string;
  notes?: string | null;
  status: string;
  createdAt: string;
  requestedBy?: { id: string; name: string; email: string };
};

export default function TaskforceQcRequestsPage() {
  const [requests, setRequests] = useState<Request[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [activeId, setActiveId] = useState<string | null>(null);
  const [actionStatus, setActionStatus] = useState("");
  const [actionError, setActionError] = useState("");
  const [assignees, setAssignees] = useState<string>("");
  const [employees, setEmployees] = useState<{ id: string; name: string }[]>([]);

  const active = useMemo(() => requests.find((r) => r.id === activeId) || null, [requests, activeId]);

  const load = async () => {
    setLoading(true);
    setError("");
    try {
      const res = await TaskforceApi.feederRequests();
      setRequests(res.feederPoints || []);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "Failed to load requests");
    } finally {
      setLoading(false);
    }
  };

  const loadEmployees = async () => {
    try {
      const res = await EmployeesApi.list("TASKFORCE");
      setEmployees(res.employees.map((e) => ({ id: e.id, name: e.name })));
    } catch {
      setEmployees([]);
    }
  };

  useEffect(() => {
    load();
    loadEmployees();
  }, []);

  const approve = async () => {
    if (!active) return;
    setActionStatus("Approving...");
    setActionError("");
    try {
      await TaskforceApi.approveRequest(active.id, {
        assignedEmployeeIds: assignees
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean)
      });
      setRequests((prev) => prev.map((r) => (r.id === active.id ? { ...r, status: "APPROVED" } : r)));
      setActionStatus("Approved");
    } catch (err) {
      setActionError(err instanceof ApiError ? err.message : "Failed to approve");
      setActionStatus("");
    }
  };

  const reject = async () => {
    if (!active) return;
    setActionStatus("Rejecting...");
    setActionError("");
    try {
      await TaskforceApi.rejectRequest(active.id);
      setRequests((prev) => prev.map((r) => (r.id === active.id ? { ...r, status: "REJECTED" } : r)));
      setActionStatus("Rejected");
    } catch (err) {
      setActionError(err instanceof ApiError ? err.message : "Failed to reject");
      setActionStatus("");
    }
  };

  return (
    <Protected>
      <ModuleGuard module="TASKFORCE" roles={["QC"]}>
        <div className="page">
          <h1>Feeder Point Requests</h1>
          {error && <div className="alert error">{error}</div>}
          {loading ? (
            <div className="muted">Loading...</div>
          ) : requests.length === 0 ? (
            <div className="muted">No requests.</div>
          ) : (
            <div className="table">
              <div className="table-head">
                <div>Name</div>
                <div>Area</div>
                <div>Status</div>
                <div>Requester</div>
                <div>Submitted</div>
                <div>Action</div>
              </div>
              {requests.map((r) => (
                <div className="table-row" key={r.id}>
                  <div>{r.feederPointName}</div>
                  <div>{r.areaName}</div>
                  <div>{r.status}</div>
                  <div>{r.requestedBy?.name || "-"}</div>
                  <div>{new Date(r.createdAt).toLocaleString()}</div>
                  <div>
                    <button className="btn btn-primary btn-sm" onClick={() => setActiveId(r.id)}>
                      Review
                    </button>
                  </div>
                </div>
              ))}
            </div>
          )}

          {active && (
            <div className="modal-backdrop">
              <div className="modal">
                <div className="modal-header">
                  <h3>Feeder Request Detail</h3>
                  <button className="icon-button" onClick={() => setActiveId(null)}>
                    x
                  </button>
                </div>
                <div className="modal-body">
                  <div className="grid grid-2">
                    <Field label="Name" value={active.feederPointName} />
                    <Field label="Area" value={active.areaName} />
                    <Field label="Zone" value={active.zoneName} />
                    <Field label="Ward" value={active.wardName} />
                    <Field label="Population Density" value={active.populationDensity} />
                    <Field label="Accessibility" value={active.accessibilityLevel} />
                    <Field label="Households" value={active.householdsCount?.toString()} />
                    <Field label="Vehicle Type" value={active.vehicleType} />
                    <Field label="Landmark" value={active.landmark} />
                    <Field label="Status" value={active.status} />
                    <Field label="Requested By" value={active.requestedBy?.name} />
                    <Field label="Submitted" value={new Date(active.createdAt).toLocaleString()} />
                  </div>
                  {active.notes ? (
                    <div style={{ marginTop: 8 }}>
                      <label>Notes</label>
                      <div className="muted">{active.notes}</div>
                    </div>
                  ) : null}

                  <div style={{ marginTop: 12 }}>
                    <label>Assign employees (comma separated IDs)</label>
                    <input className="input" value={assignees} onChange={(e) => setAssignees(e.target.value)} />
                    {employees.length > 0 && (
                      <div className="muted" style={{ marginTop: 4 }}>
                        Known IDs: {employees.slice(0, 5).map((e) => e.id).join(", ")} ...
                      </div>
                    )}
                  </div>

                  {active.photoUrl && (
                    <div style={{ marginTop: 12 }}>
                      <label>Photo</label>
                      <img src={active.photoUrl} alt="Location" style={{ maxWidth: "100%", borderRadius: 8 }} />
                    </div>
                  )}

                  {actionError && <div className="alert error">{actionError}</div>}
                  {actionStatus && <div className="alert success">{actionStatus}</div>}

                  <div className="flex gap-2" style={{ marginTop: 12 }}>
                    <button className="btn btn-success" onClick={approve}>
                      Approve
                    </button>
                    <button className="btn btn-danger" onClick={reject}>
                      Reject
                    </button>
                  </div>
                </div>
              </div>
            </div>
          )}
        </div>
      </ModuleGuard>
    </Protected>
  );
}

function Field({ label, value }: { label: string; value?: string }) {
  return (
    <div>
      <label>{label}</label>
      <div className="muted">{value || "-"}</div>
    </div>
  );
}
